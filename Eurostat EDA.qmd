...
```{r, echo=FALSE}
Demographics_NUTS2 <- read.csv('demo_r_pjangroup__custom_3760964_linear.csv') %>%
  select(geo, TIME_PERIOD, OBS_VALUE) %>%
  mutate(Population = OBS_VALUE, Year = TIME_PERIOD, NUTS2 = geo) %>%
  select(NUTS2, Year, Population)
...
Education_NUTS2 <- read.csv('edat_lfse_16__custom_3760985_linear.csv') %>%
  select(geo, TIME_PERIOD, OBS_VALUE) %>%
  mutate(Early_leavers = OBS_VALUE, Year = TIME_PERIOD, NUTS2 = geo) %>%
  select(NUTS2, Year, Early_leavers)
```
