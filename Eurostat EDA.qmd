...
```{r}
ggplot(data = GDP_Per_Capita, aes(x = GDP_capita, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.25) + labs(x = "GDP per Capita", y = "GINI") + theme_bw()
```

```{r, echo=FALSE}
GDP_Per_Capita %>%
  filter(Year == 2010) %>%
  distinct(NUTS2, GDP_capita, .keep_all = TRUE) %>%
  ggplot(aes(x = GDP_capita, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + labs(x = "GDP per Capita", y = "GINI") + theme_bw()
```

```{r, echo=FALSE}
lm(GINI_NUTS2 ~ GDP_capita, data = GDP_Per_Capita, subset = Year == 2010)
```

```{r, echo=FALSE}
lm_GDP_on_GINI <- lm(GINI_NUTS2 ~ GDP_capita, data = GDP_Per_Capita, subset = Year == 2010)
...
GDP_Per_Capita %>%
  filter(Year == 2010) %>%
  distinct(NUTS2, GDP_capita, .keep_all = TRUE) %>%
  ggplot(aes(x = GDP_capita, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + geom_abline(intercept = b0_GDP_on_GINI, slope = b1_GDP_on_GINI, color = "blue", alpha = 0.5) + labs(x = "GDP per Capita", y = "GINI") + theme_bw()
```
