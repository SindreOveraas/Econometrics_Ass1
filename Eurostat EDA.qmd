...
In these regressions we will still keep the logarithm of GDP per capita ($X_i$) as our explanatory variables.
The analysis consists of a 2^nd^ and 3^rd^ degree polynomial regressions.
...
huxreg(list("OLS" = lm20,  "2nd degree polynomial" = lm21, "3rd degree polynomial" = lm22),
...
```{r, echo=FALSE}
lm_poly_log_GDP_on_GINI <- lm(GINI_NUTS2 ~ poly(log_GDP_capita_NUTS2, 3), data = Inequity_Data)
summary(lm_poly_log_GDP_on_GINI)
```
