...
```{r, echo=FALSE}
#| label: tableGDP
#| fig-cap: GDP all countries
kable(summary(GDP_Per_Capita[,3:5]))
```

Looking at the GDP per capita table for all countries in the dataset, we find that the difference in population from min to max is big.
...
We also had some NA (Not Available) values witch we chose to ignore for the dataset.
...
In the plot above we have taken out the countries that belongs to our selection - Austria (AT), Belgium (BE), Bulgaria (BG), Croatia (HR), Italy (IT), Serbia (RS) and Sweden (SE) - and added them in to a time series from year 2000 - 2020 (X-axis) to compare the evolution of GDP per capita (Y-axis) over the given time period.
...
This might come from the fact that Serbia became an independent republic in 2006 and for that reason there has not been collected and established much info concerning GDP [@Serbia].
...
In the fourth and final part of the paper we will estimate the effect of regional development $y_j$ on regional inequality $GINIW_j$ in a penal setup.
We will more specifically estimate the following fixed effects separately and jointly on country fixed effects, year fixed effects and NUTS 2 fixed effects.
