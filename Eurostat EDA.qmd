...
Looking at the GDP per capita for all countries in the dataset in @tbl-1, we find that the difference in population from min to max is big.
The difference between Median and Mean is also relatively big which can indicate that some of the biggest regions has much larger amount of population then the rest and therefore affects the mean and pulls it higher.
The fact that the third quartile is just a few thousands away from the mean amplifies our suspicion that we have some outliers with a very high population compared to the rest.
...
The countries with the highest GDP per capita is Belgium and Sweden, where both countries has experienced growth in GDP per capita over time.
...
The following years after 2005 some of the regions have split a bit from others and had a faster growth which laded them to surpass a GDP per capita over 5000 in 2019, some reaching a GDP per capita over 7500 by the year 2019.
...
Stockholms 64,537, less then a 2x difference [@StatisticalRegionsEuropean].
...
AT323 which is Salzburg, more specifically Salzburg und Umgebung \[\@NUTSStatisticalRegions2022\], had a population of 366,866 in 2019, which is a great difference compared to SE110 - Stockholm with a population of 2,344,124 in 2019.
...
In the fixed model above the groups country, year and NUTS2 are fixed and not random or mixed.
If the the individual-specific effects are correlated with the independent variables, it is called The Fixed effects assumption.

From @tbl-25 we can see that we have 4253 observations on each fixed effect and none of the Fixed effects are notable high, and therefore don't explain a big enough proportion of the variation in the dependent variable that can be attributed to the independent variable.
