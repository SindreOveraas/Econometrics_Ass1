...
Looking at the GDP per capita for all countries in the dataset in @tbl-1, we find that the difference in population from min to max is large.
The difference between Median and Mean is also relatively large which can indicate that some of the biggest regions has much larger amount of population then the rest and therefore affects the mean and pulls it higher.
The fact that the third quartile is just a few thousand away from the mean amplifies our suspicion that we have some outliers with a very high population compared to the rest.
...
In @tbl-1 we can see that its a big difference in GDP per capita in each region, this can be caused by a population or a cluster with rich/poor individuals.
...
In the plot @fig-1 we have taken out the countries that belongs to our selection - Austria (AT), Belgium (BE), Bulgaria (BG), Croatia (HR), Italy (IT), Serbia (RS) and Sweden (SE) - and added them in to a time series from year 2000 - 2020 (X-axis) to compare the evolution of GDP per capita (Y-axis) over the given time period.
The abbreviation CC in the figure stands for Countries Combined.
Seven out of eight countries we where given was available for download trough Eurostat, the country which we are missing is Bosnia and Herzegovina.

From the plot we can see that none of the countries have data from the whole time frame, some are missing data from the first years and others are missing from the last years.
Serbia only have data from around 2017 to 2019.
This might come from the fact that Serbia became an independent republic in 2006 and for that reason has not been collecting og establishing much info around GDP [@Serbia]. The countries with the highest GDP per capita is Belgium and Sweden, where both countries has experienced growth in GDP per capita over time. Sweden also has had the biggest growth in GDP Per capita compared to the other countries. From the graphs in @fig-1, the three countries with highest GDP per capita had a noticeable decline around 2008, which is related to the financial crises at this time. Sweden also experiences the largest dip around this time [@EconomicRecoveryEU].
Four of the five countries on the bottom also has experienced growth, just not as significant as Sweden and Belgium.
Austria had a big downfall in GDP per capita from year 2000 to 2001, but has had a stable growth since then.

### Bulgaria GDP

@fig-2 shows the GDP per capita on a NUTS3 level for Bulgaria alone.
...
The following years after 2005 some of the regions have split a bit from others and had a faster growth which laded them to surpass a GDP per capita over 5,000 in 2019, some reaching a GDP per capita over 7,500 by the year 2019.

Using 2010 as a base point which is after the financial crisis, we found that the three regions with highest GDP per capita is BG411 (Sofia), BG344 (Stara Zagora) and BG331 (Varna), all major cities in Bulgaria.
These Regions also managed to maintain a good growth in GDP per capita through the years until 2019 [@StatisticalRegionsEuropean].
This is shown in @tbl-2.

The three regions with lowest GDP per capita is BG325 (Silistra), BG311 (Vidin) and BG342 (Silven) where BG325 and BG311 both are cities located north in Bulgaria, as shown in @tbl-3.
BG342 is a larger city located more in the middle of Bulgaria a bit southeast [@StatisticalRegionsEuropean].

### Belgium GDP

In Belgium @fig-3 shows that it is more variance in GDP per capita from region to region.
Most regions have had somewhat of a similar growth trough year 2000 until 2018 before a dip from 2018 to 2020.
This decline could be explained by the government formation that started to take place in 2018.
...
The three regions with the largest GDP in Belgium after the financial crises in 2010, shown in @tbl-4, is BE100 (Brussel), BE211 (Antwerpen) and BE241 (Halle-Vilvoorde) [@StatisticalRegionsEuropean] .
...
@tbl-5 show the three regions with the smallest GDP per capita in Belgum by 2010 was BE334 (Waremme), BE353 (Philippeville) and BE345 (Virton) [@StatisticalRegionsEuropean].
Philippeville and Virton is both located north and border to France.
Waremme is located more mid-east.

### Austria GDP

With @fig-4 we can see that Austria has a more aggregated graphs, even though it is a gap between highest and lowest, the GDP per capita development is more equal.
...
AT312 (Linz-Wels), AT130 (Wien) and AT323 (Salzburg) is the Regions with the highest scoring GDP per capita in 2010, as displayed by @tbl-6.
They have held a similar pattern and GDP per capita from 2000 to 2020 [@StatisticalRegionsEuropean].

@tbl-7 show the three lowest scoring regions are AT125 (Weinviertel), AT111 (Mittelburgenland) and AT313 (Mühlviertel) witch all is more greener areas.
...
@fig-5 illustrate graphs for Croatia that show the financial crisis also had an impact on the GDP per capita in 2008 until 2012/2013 before the GDP per capita started to increase again before it once again decreases around 2018.
We can see that region HR036 and HR031 has GDP per capita that lay a bit higher than the other regions for the most part.
However, region HR037 and HR032 also had some good years.
Croatia has also fewer regions with available GDP data on a NUTS 3 level than the other countries.

The top three regions in 2010 measured by GDP per capita is HR036 (Istarska županija), HR031 (Primorsko-goranska županija) and HR037 (Dubrovnik-Neretva) shown in [@tbl-8; @StatisticalRegionsEuropean].
Istarska županija is big on processing industry, tourism and trade [@Economy].
Dubrovnik-Neretva is the capital of Croatia and are also big on tourism, but also makes a living out of hospitality industry, agriculture and maritime industry [@Economya].

On the other side, the three regions with lowest GDP per capita by 2010 is HR034 (Šibensko-kninska županija), HR032 (Ličko-senjska županija) and HR035 (Splitsko-dalmatinska županija) shown in @tbl-9.
All are located south in Croatia by the coast and boarders to Bosnia-Herzegovina [@StatisticalRegionsEuropean] .

### Italy GDP

As illustrated in @fig-9 Italy consists of many NUTS3 Regions.
We piced out only the 500 biggest and used them in our plot.
Some of the regions in the plot do not have GDP per capita data for the whole-time frame.
...
We can also see that it is some what of a decline around 2011 too.
...
@tbl-10 show that TC4C (Milano), ITH10 (Bolzano-Bozen) and ITI43 (Rome) are the three regions in Italy with the highest GDP per capita by 2010 [@StatisticalRegionsEuropean].
...
@tbl-11 show that out of the 500 Regions we picked out, ITG4 (Agrigento), ITF48 (Barletta-Andria-Trani) and ITF64 (Catanzaro) had the lowest GDP per capita in 2010 [@StatisticalRegionsEuropean].
All three regions are located South in Italy.
Agrigentos economy is focused around Sulfur and potash mining, tourism and agriculture [@AgrigentoItalyBritannica].
...
Looking at @fig-7, 2018 seems to be a breaking point for the GDP per capita where some regions experiences growth, while other experiences a decline in GDP Per capita.

The three regions with the highest GDP per capita in Serbia, here measured year 2017 in @tbl-12 was RS221 (Borska Oblast), RS214 (Moravička Oblast) and RS226 (Pirotska Oblast) [@StatisticalRegionsEuropean].

@tbl-13 show the three lowest scoring regions in terms of GDP per capita in Serbia of the year 2017 was RS228 (Pčinjska Oblast), RS227 (Pirotska Oblast) and RS224 (Jablanička Oblast) [@StatisticalRegionsEuropean].

### Sweden GDP

The statistics of @fig-8 show Stockholm as superior to the other regions both in GDP, GDP per capita and population.
...
What is notable her is that the neighbor region SE122 (Sódermanland) which borders Stockholm, is second to last at the bottom with half the GDP per Capita compared to Stockholm.
We believe this has a connection to the employment rates and there would be a bigger attraction for workers to employ in Stockholm.
...
Stockholms 64,537, less than a 2x difference [@StatisticalRegionsEuropean].

As previously we chose 2010 as a benchmark, where the recovery from the financial crisis was in route.
Stockholm had a GDP per capita of 56,036 at that time while Norrbottens came in second with 43,768 and Vastra Gotaland at third with 38,740, as shown in @tbl-14.
@tbl-15 show Swedish regions with the lowest GDP per capita in 2010 were fairly close, and little that differed them from each other as well as some other regions.

### 10 regions with highest GDP per capita

Comparing the regions with the highest GDP per capita between the countries in @fig-9, we see BE100 (Brussels) with a GDP that was rising well above 70,000 before the corona pandemic, and was slightly lower than 70 000 in 2020.
From this dataset among the highest GDP per capita regions, it was only Brussels and Antwerp that had data available until 2020.
AT323 which is Salzburg, more specifically Salzburg und Umgebung, had a population of 366,866 in 2019, which is a great difference compared to SE110 - Stockholm with a population of 2,344,124 in 2019.
A considerable difference, showing how the GDP per capita is fairly close to each other even though the aforementioned population difference.

### 10 regions with lowest GDP per capita

The 10 regions with the lowest GDP per capita, illustrated in @fig-10, show us they are *all* located in Bulgaria**.** Eight of the lowest GDP per capita regions are located in NUTS 1 - BG3 (Northern & Eastern Bulgaria).
BG 3 is divided into 4 NUTS 2 regions: (BG31) Northwestern, (BG32) Northern Central, (BG33) Northeastern, (BG34) Southeastern.
BG31 has three provinces that are on our top 10 list; Vidin Province (311), Montana Province(312) and Pleven Province (314).
...
The interesting part of this diagram is that around 2004, the GDP per capita of the regions were fairly aligned with each other at 2000, before they started ascending and increasing the gap.
We want to clarify that there was insufficient data when it came to Bosnia & Herzegovina, which could have been a contender for the lowest GDP per Capita in some regions.
Same goes for Serbia, where we chose to withhold them from this top 10 list because a lack of information pre- 2017.
...
The summary statistics in @tbl-16 give us the descriptive figures for all our selected countries combined, throughout the whole period of 2000 - 2020.
...
Based on the data of @tbl-17 we see that all of the countries are collectively within adequate equality or better.
...
The capital of Bulgaria, Sofia, has more than twice of the average GDP per capita of the remaining regions, and for some of the lower regions, more than 5 times the amount.

@fig-11 show the density distribution of all Gini coefficients in all selected countries.
We can observe that the density plot matches our summary statistics of the coefficients.
The density is at its peak at approximately 0.03, reaching almost 10 in density.
This signalizes that there is adequate equality between regions.

In the scatterplot @fig-12 we see 4 countries touching the 0.00 Gini point, symbolizing perfect equality for some regions, with one stretching beyond 0.2 going towards adequate equality.
We have however chosen to ignore Gini coefficients of the value 0, considering these observations are most likely due to missing data.
...
We can take a closer look at Sweden in @fig-13 and we see their regions and how their Gini index has been throughout the years.
Comparing the @fig-13 of Sweden to @fig-14 of Belgium, we see there is a bigger diversity and inequality in Belgium compared.
...
This visualization of @fig-15 shows us how the plot excelled during the years and created a bigger gap with the years.
The overall mean also increased with the years implying they are moving further away from equality.
...
Further on when we went deeper into detail, we saw the Gini coefficients of the countries and their regions, we can concluded that they were all within the limit of adequate equality, while some were close satisfactory equality.
...
#| label: tbl-18
#| tbl-cap: "Regional development on regional inequality CC"
...
As shown in the regression table @tbl-18, we have the intercepts and slope coefficient of all separate regions of regional economic development ($X_i$) on regional inequality ($Y_i$) (excluding Serbia (RS) because of insufficient data).
...
To determine the goodness of fit for the estimated simple regression models in @tbl-18 we look at the coefficient of determination $R^2$.
...
This can also be visualized with scatter plot as shown in @fig-17.
...
We have plotted the residuals for our simple regression model in the scatter plot, as illustrated in @fig-18.
...
1.  Linearity: There must be a linear relationship between the dependent ($Y_i$) and independent ($X_i$) variables. As shown on our scatter plots with the fitted line, we see that the fitted line represents all plots as good as possible. We can see from the scatterplot that the relationship between GINI and GDP per capita variates, we have some outliers with a much higher standard error then the rest of the selection
2.  Sample variation: There is variation in the independent variable ($X_i$).
3.  Random sampling: There is a random sample from the population of interest. Our data is collected from Eurostat, which is a reliable source in terms of random sampling. We have omitted NA values and countries that did not belong to our dataset. For some of the countries we also have omitted some observations because these countries had a large amount of collected observations
4.  Exogeneity: The explanatory variable ($X_i$) is exogenous, i.e., it is not influenced by any other variables in the explanatory model. This is true in this case, GDP Per capita is not influenced, but rather the one that could influence the other variables.
5.  Homoskedasticity: The error term ($ε_i$) has the same variance for each value of the independent variable ($X_i$). The scatter plot shows that the error term does not have the same variance like this assumption wishes for.
6.  Normality: The population error term ($ε_i$) is normally distributed. As our Error term does not equal zero this assumption is not for filled in our dataset.
...
@tbl-20 and @tbl-21 have two multiple regression models and three simple regression models.
Model 3 and 5 are the multiple regression models, whereas 1, 2 and 4 are simple regression models.

@tbl-20 show the multiple regression models for Austria show a coefficient of determination $R^2$ of 0.794 for Model 3 and 0.941 for Model 5, respectively.
Model 3 concerns a multiple regression of percentage of the population with higher education, the number of railways and motorways, and GDP per capita as the independent variables, and the Gini coefficient as the dependent variable.
...
In @tbl-21 we see a much lower degree of explanation for the models, than that of Austria.
...
Like the second, this third part will deal with regions of NUTS2 level in our subset of countries.

### Effect of development for different subsets

The first thing we will explore in this third part of the paper is to determine if the effect of development is significantly different for different subset of countries in our data set.
To determine this we will create a dummy variable.
A dummy variable is a variable used to explain qualitative values in a regression model.
Dummy variables usually take binary values, i.e. 0 or 1.
We have chosen to use the data of Italy as the dummy variable, considering this is the country with most observations of 2010, and it is therefore the appropriate variable to compare with the rest of the data from all the other countries.
We choose to multiply our dependent variable of the Gini coefficient by 10,000, so we get a better visualization of the slope coefficient.

As seen from @tbl-22 we have an appropriate distribution of observations, with Italy having 34 observations and the other countries combined having 37.
We also see that the correlation between the dependent and the independent variable for the two regression models, that is the effect of regional economic development ($X_i$) on regional inequality ($Y_i$), are very similar.
For Italy only we have a coefficient of determination of 0.33, compared to the other combined countries of 0.316.
Based on this it suggests that the effect of regional economic development is not significantly different for different subsets of countries in our data set.
The slope coefficient does however tell us that the effect is negative for Italy, and positive for the other combined countries, i.e. that higher regional development in Italy has the tendency to lead to greater inequality.
For the other countries combined this trend is the opposite, that higher regional development decreases inequality.
...
The scatterplot for the regression of the CC with the log of GDP per capita data is shown in @fig-19.

```{r, echo=FALSE}
#| label: fig-19
#| fig-cap: "Log of GDP per capita on Gini"
...
The analysis consists of 2^nd^ and 3^rd^ degree polynomial regressions in @tbl-24.
...
Polynomial regression is however sensitive to outliers, which seems to be the case for our estimates.
The models are visualized by @fig-20 and @fig-21.
...
```{r, echo=FALSE}
#| label: fig-20
#| fig-cap: "Second degree polynomial"
Inequity_Data %>%
...
```{r, echo=FALSE}
#| label: fig-21
#| fig-cap: "Third degree polynomial"
Inequity_Data %>%
...
When doing this test, the goal is to assert whether there is heteroscedasticity or homoscedasticity, which then will tell us whether the results of the regression are reliable or not.
...
This output from the second degree polynomial regression shows us similar data as far as keeping the null-hypothesis and assuming *homoscedasticity* for the residuals.
...
Our fourth, and second simple regression going through the Breusch Pagan Test, shows us a p-value (0.8762755) above our alpha (0.05) again, implying the residuals are *homoscedastistic* and our null-hypothesis stays.

We followed through with our given recommendations for how to solve this task of looking for heteroscedasticity, giving us the results as seen above.
But we would like to add a note; According to Dr. Nisha Arora, *the Breusch Pagan Test assumes heteroscedasticity is a linear function of the independent variables.* Where if one would fail to find heteroscedisticity with the Breuch Pagan test, it would not necessarily mean that there is not a nonlinear relation between the error variance and the independent variables.
Further it seems to be recommended to use the *White Test,* where as this test gives us a more flexible functional form that is suited for recognizing a greater amount of patterns for heteroscedasticity.
The *White Test* also lets the independent variables to have a interactive and nonlinear effect on the error variance, and is a good tool as a safety measure @aroraAnswerWhatCriterion2016.
This note was added due to the fact that we are actually looking at a theoretical nonlinear model, but through the transformation of data, we can still consider this as a linear model.
Hence, this is why we did not do the White Test as an extra safety measure.
...
We will then see a reduction of middle class, where those who once were there, would either enter the upper class or the lower class, usually they will fall in to the latter category and the gap will expand.
...
Where in the Scandinavian countries one might be jobless and still make a living through their welfare systems, more specifically social securities.
...
Social welfare is by all means, also in Bulgaria, but to a different degree, which we also believe could be on of the reasons for why the number of people who have left Bulgaria to work and live in other countries has doubled since they got rights to move freely in the EU @networkHowManyBulgarians.
...
Causality appears when it is a velar cause-effect relationship between two variables.
There is causation when A causes outcome B.
a common mistake is to infer causality when correlation is present, but correlation is only a relationship.

The difference between Correlation and Causality is that when it is a correlation, we don't know which way the relationship goes.
It can also be a third variable that effects the correlation.
In Causality variable A can predict variable B.
We know the direction of the relationship.
Its not possible to search for causality with the regression, it can only be used if a casual relationship is assumed.

To establish causality, two predictions need to be fulfilled:

1.  There needs to be a significant relationship, significant correlation.
2.  It needs to be a temporal sequence of the variables, variable A took place before variable B. Or if it is a theoretical justified and plausible theory which direction the causal relationship goes

From our models we find that the correlation between log GDP per Capita and Gini is not significant in total for all countries in the dataset.
This means that we cannot for fill the first point to Causality.
Furthermore, if the correlation where to be significant, we would argue that A (GDP per Capita) could not predict B (Gini/Inequality).
Taking a starting point in that Gini is measured out from different regions, this means that it based on location and if GDP per Capita where to stay the same, the population of the regions could still move around resulting in changes in Inequality.

For example; if the population in the regions split equally around to make all regions have the same GDP per capita, the inequality between the regions would end up becoming lower.
So there could be a correlation between X and Y, but Z could be the explanation for Y.

## Panel estimates

So far we have mostly been using time-series data to determine how regional development has an effect on regional inequality.
In the last part of the paper we want to explore how the data in a panel setup might determine the statistical outcome of the effect of regional development $y_j$ on regional inequality $GINIW_j$.
For this part we have chosen to base our analysis on the NUTS 3 level, i.e., use the values of the variables such as GDP per capita and Gini coefficients on the NUTS 3 level.
We will more specifically estimate the following fixed effects: Country fixed effects, year fixed effects, NUTS 2 fixed effects as well as these variables fixed jointly.
The fixed effect models will only focus on variation within the individual observations we have in our dataset.

```{r, echo=FALSE}
Panel_Inequity_Data <- GDP_Per_Capita %>%
  distinct(Year, NUTS2, GINI_NUTS2, GDP_capita, Population) %>%
  mutate(GDP_capita = GDP_capita) %>%
  mutate(GINI_NUTS3 = GINI_NUTS2*100000) %>% 
  mutate(NUTS = substr(NUTS2,1,2)) %>%
  select(Year, NUTS, NUTS2, Population, GDP_capita, GINI_NUTS2, GINI_NUTS3)
```

```{r, echo=FALSE}
plm1 <- plm(GINI_NUTS3 ~ GDP_capita, data = Panel_Inequity_Data, index = c("NUTS"), model = "within") %>% 
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
```

```{r, echo=FALSE}
plm2 <- plm(GINI_NUTS3 ~ GDP_capita, data = Panel_Inequity_Data, index = c("Year"), model = "within", effect = "twoways") %>% 
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
```

```{r, echo=FALSE}
plm3 <- plm(GINI_NUTS3 ~ GDP_capita, data = Panel_Inequity_Data, index = c("NUTS2"), model = "within") %>% 
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
```

```{r, echo=FALSE, warning=FALSE}
plm4 <- plm(GINI_NUTS3 ~ GDP_capita, data = Panel_Inequity_Data, index = c("NUTS", "Year", "NUTS2"), model = "within") %>% 
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
```

In the fixed model the variables country, year and NUTS2 are fixed and not random or mixed.
From @tbl-25 we can see that we have 4253 observations on each fixed effect.
None of the correlations of the fixed effects are notably high, and therefore don't explain a big proportion of the variation in the dependent variable that can be attributed to the independent variable.
The only fixed effect that seem to have some form of correlation is the fixed effect on the variable year, which have an $R^2$ of 0.163 and a p-value \< 0.01 that tells us the correlation is statistically significant.

## Final conclusion

We conclude this paper by shortly explaining our findings concerning our main hypothesis, which is the following; That we can observe a significant and positive effect of development on regional inequality for our set of countries.
It is clear that through development in our countries that there has been a positive effect on the gap of inequality in their respective regions.
In the sense that the gap and the Gini coefficient has increased, for that matter, the overall affect is negative.
With a situation with economical inequity, a pattern might assert itself if not handled accordingly.
The leading regions will continue to develop and advance their own region while the lower regions will struggle to follow leading regions.
This can result in an even bigger gap, and lower regions are usually prone to have smaller opportunities in form of education, health and general social securities.
Factors that can be considered crucial to being able to evolve at all.
Looking through our data, tables and figures it is clear what the common denominator is for being the leading region in all countries, where the capital, and the business cluster reside.
These countries and their development is reflected by each of their governments policies concerning the countries.
Where countries with stronger social policies, such as Sweden, will put more pressure on lifting the lowered regions and sharing wealth with the goal of decreasing the inequality gap.
In most cases of this paper, the development of the respective countries apparently had a negative effect on the inequality in the regions.
...
```{r, echo=FALSE}
#| label: tbl-1
#| tbl-cap: "GDP per capita all countries"
kable(summary(GDP_Per_Capita[,3:5]))
```

```{r, echo=FALSE}
#| label: fig-1
#| fig-cap: "GDP per capita CC 2000-2020"
...
```{r, echo=FALSE}
#| label: fig-2
#| fig-cap: "GDP per capita Bulgaria"
...
```{r, echo=FALSE}
#| label: tbl-2
#| tbl-cap: "Highest GDP per capita Bulgaria 2010"
...
```{r, echo=FALSE}
#| label: tbl-3
#| tbl-cap: "Lowest GDP per capita Bulgaria 2010"
kable(GDP_Per_Capita %>%
  filter(NUTS == "BG", Year == 2010) %>%
  select(Region, GDP_capita) %>%
...
```{r, echo=FALSE}
#| label: fig-3
#| fig-cap: "GDP per capita Belgium"
...
```{r, echo=FALSE}
#| label: tbl-4
#| tbl-cap: "Highest GDP per capita Belgium 2010"
kable(GDP_Per_Capita %>%
...
```{r, echo=FALSE}
#| label: tbl-5
#| tbl-cap: "Lowest GDP per capita Belgium 2010"
kable(GDP_Per_Capita %>%
  filter(NUTS == "BE", Year == 2010) %>%
  select(Region, GDP_capita) %>%
  slice_min(GDP_capita, n = 3))
...
```{r, echo=FALSE}
#| label: fig-4
#| fig-cap: "GDP per capita Austria"
...
```{r, echo=FALSE}
#| label: tbl-6
#| tbl-cap: "Highest GDP per capita Austria 2010"
kable(GDP_Per_Capita %>%
...
```{r, echo=FALSE}
#| label: tbl-7
#| tbl-cap: "Lowest GDP per capita Austria 2010"
kable(GDP_Per_Capita %>%
  filter(NUTS == "AT", Year == 2010) %>%
  select(Region, GDP_capita) %>%
  slice_min(GDP_capita, n = 3))
...
```{r, echo=FALSE}
#| label: fig-5
#| fig-cap: "GDP per capita Croatia"
...
```{r, echo=FALSE}
#| label: tbl-8
#| tbl-cap: "Highest GDP per capita Croatia 2010"
kable(GDP_Per_Capita %>%
...
```{r, echo=FALSE}
#| label: tbl-9
#| tbl-cap: "Lowest GDP per capita Croatia 2010"
kable(GDP_Per_Capita %>%
  filter(NUTS == "HR", Year == 2010) %>%
  select(Region, GDP_capita) %>%
  slice_min(GDP_capita, n = 3))
...
```{r, echo=FALSE}
#| label: tbl-10
#| tbl-cap: "Highest GDP per capita Italy 2010"
kable(GDP_Per_Capita %>%
...
```{r, echo=FALSE}
#| label: tbl-11
#| tbl-cap: "Lowest GDP per capita Italy 2010"
kable(GDP_Per_Capita %>%
  filter(NUTS == "IT", Year == 2010) %>%
  select(Region, GDP_capita) %>%
  slice_min(GDP_capita, n = 3))
...
```{r, echo=FALSE}
#| label: tbl-12
#| tbl-cap: "Highest GDP per capita Serbia 2010"
kable(GDP_Per_Capita %>%
...
```{r, echo=FALSE}
#| label: tbl-13
#| tbl-cap: "Lowest GDP per capita Serbia 2010"
kable(GDP_Per_Capita %>%
  filter(NUTS == "RS", Year == 2017) %>%
  select(Region, GDP_capita) %>%
  slice_min(GDP_capita, n = 3))
...
```{r, echo=FALSE}
#| label: tbl-14
#| tbl-cap: "Highest GDP per capita Sweden 2010"
kable(GDP_Per_Capita %>%
...
```{r, echo=FALSE}
#| label: tbl-15
#| tbl-cap: "Lowest GDP per capita Sweden 2010"
kable(GDP_Per_Capita %>%
  filter(NUTS == "SE", Year == 2010) %>%
  select(Region, GDP_capita) %>%
  slice_min(GDP_capita, n = 3))
...
```{r, echo=FALSE}
#| label: fig-9
#| fig-cap: "10 regions with highest GDP per capita"
...
```{r, echo=FALSE}
#| label: fig-10
#| fig-cap: "10 regions with lowest GDP per capita"
...
```{r, echo=FALSE}
#| label: tbl-16
#| tbl-cap: "GDP Gini coefficient CC"
...
```{r, echo=FALSE}
#| label: tbl-17
#| tbl-cap: "GDP Gini coefficient CC 2010"
kable(summary(GDP_Per_Capita %>%
  filter(Year == 2010) %>%
  select(GINI_NUTS2)))
```
...
```{r, echo=FALSE}
#| label: fig-14
#| fig-cap: "Scatterplot of Gini Belgium"
GDP_Per_Capita %>%
  filter(NUTS == "BE") %>%
  distinct(NUTS2, Year, .keep_all = TRUE) %>%
  ggplot(aes(x = Year, y = GINI_NUTS2, fill = NUTS2, color = NUTS2)) + geom_point(lwd = 0.7) + labs(x = "Year", y = "GINI") + theme_bw()
...
```{r, echo=FALSE}
#| label: fig-15
#| fig-cap: "Scatterplot of Gini Bulgaria"
GDP_Per_Capita %>%
  filter(NUTS == "BG") %>%
  distinct(NUTS2, Year, .keep_all = TRUE) %>%
  ggplot(aes(x = Year, y = GINI_NUTS2, fill = NUTS2, color = NUTS2)) + geom_point(lwd = 0.7) + labs(x = "Year", y = "GINI") + theme_bw()
...
```{r, echo=FALSE}
#| label: fig-16
#| fig-cap: "Gini & GDP per capita IT"
IT2010 %>%
...
```{r, echo=FALSE}
#| label: tbl-22
#| tbl-cap: "Italy compared to CC"
Inequity_Data$Italy <- ifelse(Inequity_Data$NUTS == "IT", 1,0)
Inequity_Data$Other <- ifelse(Inequity_Data$NUTS == "IT", 0,1)

Inequity_Data <- Inequity_Data
lm23 = lm(GINI_NUTS2*10000 ~ GDP_capita_NUTS2, filter(Inequity_Data, Italy == 1))
lm24 = lm(GINI_NUTS2*10000 ~ GDP_capita_NUTS2, filter( Inequity_Data, Italy == 0))

huxreg(list("Italy" = lm23, "All other countries" = lm24), statistics = c(N = "nobs", R2 = "r.squared", FStat = "statistic", PValue = "p.value"), note = "Note: {stars} T statistics in brackets.")
```

```{r, echo=FALSE, warning=FALSE}
#| label: tbl-23
#| tbl-cap: "Log of GDP per capita on Gini"
...
```{r, echo=FALSE}
#| label: tbl-24
#| tbl-cap: "Polynomial GDP per capita on Gini"
...
```{r, echo=FALSE}
#| label: tbl-25
#| tbl-cap: "Panel estimates"
huxreg(list( "Fixed effect Country" = plm1, "Fixed effect Year" = plm2, "Fixed effect NUTS2" = plm3, "Fixed effect jointly" = plm4),
  statistics = c(N = "nobs", R2 = "r.squared", FStat = "statistic", PValue = "p.value"),
