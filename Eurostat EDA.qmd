...
```{r, echo=FALSE}
lm_GDP_on_GINI_BE <- lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = BE2010)

b0_GDP_on_GINI_BE <- lm_GDP_on_GINI_BE$coefficients[1]
b1_GDP_on_GINI_BE <- lm_GDP_on_GINI_BE$coefficients[2]

BE2010 %>%
  ggplot(aes(x = GDP_capita_NUTS2, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + geom_abline(intercept = b0_GDP_on_GINI_BE, slope = b1_GDP_on_GINI_BE, color = "blue", alpha = 0.5) + labs(x = "GDP per Capita BE", y = "GINI BE") + theme_bw()
```
...
That is therefore the reason we will use the log of GDP per capita in our new estimates in the linear regression model, and see if this fits our model better.
...
huxreg(list("AT" = lm13, "BE" = lm14,  "BG" = lm15, "HR" = lm16, "IT" = lm17, "SE" = lm18, "CC" = lm19), statistics = c(N = "nobs", R2 = "r.squared"), coefs = c(log_GDP_capita_NUTS2 = "log_GDP_capita_NUTS2", const. = "(Intercept)"), note = "Note: {stars} T statistics in brackets.")
```
...
Furthermore, we would like to present a polynomial regression analysis to determine if this will fit and explain our data better.

```{r, echo=FALSE}
lm_poly_GDP_on_GINI <- lm(GINI_NUTS2 ~ poly(GDP_capita_NUTS2, 5), data = Inequity_Data)
summary(lm_poly_GDP_on_GINI)
```

```{r, echo=FALSE}
Inequity_Data %>%
  ggplot(aes(x = GDP_capita_NUTS2, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + stat_smooth(method = "lm", formula = y ~ poly(x, 5), se = TRUE, color = "blue", alpha = 0.2) + labs(x = "GDP per Capita", y = "GINI") + theme_bw()
```
