...
According to Lessmann and Seidel, the level of economic development is measured by the logarithm of GDP per capita [@lessmannRegionalInequalityConvergence2017].
We will therefore use the log of GDP per capita in our further calculations.
...
```{r, echo=FALSE}
lm1 = lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = AT2010)
lm2 = lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = BE2010)
lm3 = lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = BG2010)
lm4 = lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = HR2010)
lm5 = lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = IT2010)
lm6 = lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = SE2010)
lm7 = lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = Inequity_Data)
```
...
```{r, echo=FALSE, warning=FALSE}
huxreg(list("AT" = lm1, "BE" = lm2,  "BG" = lm3, "HR" = lm4, "IT" = lm5, "SE" = lm6, "CC" = lm7), statistics = c(N = "nobs", R2 = "r.squared"), coefs = c(log_GDP_capita_NUTS2 = "log_GDP_capita_NUTS2", const. = "(Intercept)"), note = "Note: {stars} T statistics in brackets.")
```

As shown in the regression table above, we have the intercepts and slope coefficient of all separate regions of regional economic development ($X_i$) on regional inequality ($Y_i$) (excluding Serbia (RS) because of insufficient data).
...
The negative slope coefficients of the regression analysis show us that there is a tendency for less regional inequality in Austria, Belgium and Sweden when regional economic development increases.
Using Austria as an example, with the slope coefficient of -0.122, the dependent variable of regional inequality ($Y_i$) will decrease with 0.122%, when the independent variable of regional economic development ($X_i$) increases by 1%.
...
For all the countries combined, we observe a negative slope coefficient of -0.031, implying once again that increase in economic development reduces inequality.
...
```{r, echo=FALSE}
lm_GDP_on_GINI_BG <- lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = BG2010)

b0_GDP_on_GINI_BG <- lm_GDP_on_GINI_BG$coefficients[1]
b1_GDP_on_GINI_BG <- lm_GDP_on_GINI_BG$coefficients[2]

BG2010 %>%
  distinct(NUTS2, log_GDP_capita_NUTS2, .keep_all = TRUE) %>%
  ggplot(aes(x = log_GDP_capita_NUTS2, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + geom_abline(intercept = b0_GDP_on_GINI_BG, slope = b1_GDP_on_GINI_BG, color = "blue", alpha = 0.5) + labs(x = "log - GDP per Capita BG", y = "GINI BG") + theme_bw()
```

For all the countries combined (CC) we have the coefficient of determination of 0.157, i.e. that the independent variable of economic regional growth, explains around 15% of the variance in the dependent variable regional inequality.
This can also be visualized with scatter plots as shown below.

```{r, echo=FALSE}
Inequity_Data %>%
  distinct(NUTS2, log_GDP_capita_NUTS2, .keep_all = TRUE) %>%
  ggplot(aes(x = log_GDP_capita_NUTS2, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + labs(x = "log - GDP per Capita", y = "GINI") + theme_bw()
```

```{r, echo=FALSE}
lm_GDP_on_GINI <- lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2, data = Inequity_Data)
...
Inequity_Data %>%
  distinct(NUTS2, log_GDP_capita_NUTS2, .keep_all = TRUE) %>%
  ggplot(aes(x = log_GDP_capita_NUTS2, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + geom_abline(intercept = b0_GDP_on_GINI, slope = b1_GDP_on_GINI, color = "blue", alpha = 0.5) + labs(x = "log - GDP per Capita", y = "GINI") + theme_bw()
```
...
Inequity_Data %>%
  ...
  geom_segment(aes(x = log_GDP_capita_NUTS2, xend = log_GDP_capita_NUTS2, y = GINI_NUTS2, yend = Predicted), color = "red", alpha = 0.4) +
                 geom_abline(intercept = b0_GDP_on_GINI, slope = b1_GDP_on_GINI, color = "blue", alpha = 0.5) +
                 labs(x = "log - GDP per Capita", y = "GINI") + theme_bw()
```

### Classic assumptions OLS
...
GDP per capita does not account for or measure the social and environmental costs of production, and other social issues such as politics and culture [@unitednationsstatisticsdivisionGROSSDOMESTICPRODUCT2007].
...
```{r, echo=FALSE}
MRM_NUTS2 <- lm(GINI_NUTS2 ~ log_GDP_capita_NUTS2 + Life_expectency + Tertiary_educ + Number_of_MWAY_and_RL, data = Inequity_Data)
summary(MRM_NUTS2)
```
