...
```{r, echo=FALSE, warning=FALSE}
huxreg(list("Model 1" = lm1, "Model 2" = lm2,  "Model 3" = lm3, "Model 4" = lm4, "Model 5" = lm5, "Model 6" = lm6, "Model 7" = lm7), statistics = c(N = "Nobs", R2 = "R.Squared"), coefs = c(log_GDP_capita_NUTS2 = "log_GDP_capita_NUTS2", const. = "(Intercept)"), note = "Note: {stars} T statistics in brackets.")
```

As shown in the regression table above, we have the intercepts and slope coefficient of all separate regions of regional economic development (X) on regional inequity (Y) (excluding Serbia (RS) because of insufficient data).
The last model in the table (Model 7) is generated from the combined data from all regions regardless of country.
...
GDP per capita does not account for or measure the social and environmental costs of production, among other things [@unitednationsstatisticsdivisionGROSSDOMESTICPRODUCT2007].
