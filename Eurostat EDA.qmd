...
```{r, echo=FALSE}
lm_GDP_on_GINI_BG <- lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = BG2010)

b0_GDP_on_GINI_BG <- lm_GDP_on_GINI_BG$coefficients[1]
b1_GDP_on_GINI_BG <- lm_GDP_on_GINI_BG$coefficients[2]

BG2010 %>%
  ggplot(aes(x = GDP_capita_NUTS2, y = GINI_NUTS2)) + geom_point(color = "maroon", alpha = 0.7) + geom_abline(intercept = b0_GDP_on_GINI_BG, slope = b1_GDP_on_GINI_BG, color = "blue", alpha = 0.5) + labs(x = "GDP per Capita BG", y = "GINI BG") + theme_bw()
```
